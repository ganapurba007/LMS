...
<div class="md-page-header mb-4">
    <div class="md-page-title">
        ...
        <div>
            <h5 class="md-title">Edit Pengguna: <span style="color:var(--tblr-primary,#206bc4);">{{ $user->name }}</span></h5>
        </div>
    </div>
...
@if($errors->any())
    <div class="md-alert danger mb-4">
        <i class="ti ti-alert-triangle"></i> Terjadi kesalahan pada formulir. Silakan periksa kolom yang ditandai.
        <button class="md-alert-close" onclick="this.closest('.md-alert').remove()"><i class="ti ti-x"></i></button>
    </div>
@endif
...
<div class="md-form-card">
    ...
    <form method="POST" action="{{ route('admin.users.update', $user) }}">
        ...
        <div class="md-form-body">
            <div class="row g-3">
                ...
                <div class="col-12 col-md-4">
                    <label for="nip" class="md-form-label">NIP / NISN</label>
                    <input type="text" id="nip" name="nip"
                        class="form-control @error('nip') is-invalid @enderror"
                        value="{{ old('nip', $user->nip) }}" placeholder="123456789...">
                    <div class="form-text text-muted" style="font-size:.75rem;">NIP untuk Guru atau NISN untuk Siswa.</div>
                    @error('nip')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>

                <div class="col-12 col-md-4">
                    <label for="role_id" class="md-form-label">Role <span class="text-danger">*</span></label>
                ...
                <div class="col-12 col-md-4">
                    <label for="class_id" class="md-form-label">Kelas</label>
                    <select id="class_id" name="class_id" class="form-select @error('class_id') is-invalid @enderror">
                        <option value="">-- Pilih Kelas (Siswa) --</option>
                        @foreach($classes as $cls)
                            <option value="{{ $cls->id }}" {{ (string) old('class_id', $user->class_id) === (string) $cls->id ? 'selected' : '' }}>
                                {{ $cls->name }}
                            </option>
                        @endforeach
                    </select>
                    <div class="form-text text-muted" style="font-size:.75rem;">Khusus bagi pengguna berperan Siswa.</div>
            ...
            <div class="mb-3">
                <label class="md-form-label mb-1">Ubah Password (Opsional)</label>
                <div class="text-muted mb-2" style="font-size:.75rem;">Kosongkan jika tidak ingin mengganti password akun pengguna.</div>
                <div class="row g-3">
                    <div class="col-12 col-md-6">
                        <input type="password" id="password" name="password"
                            class="form-control @error('password') is-invalid @enderror"
                            placeholder="Password Baru (min. 6 karakter)" autocomplete="new-password">
        ...
        <div class="md-form-footer d-flex flex-wrap align-items-center justify-content-between gap-2">
            <a href="{{ route('admin.users.index') }}" class="md-btn-light">Batal</a>
            <div class="d-flex align-items-center gap-2">
                <button type="button" class="md-btn-warning" data-bs-toggle="modal" data-bs-target="#modalResetConfirm">
                    <i class="ti ti-key"></i> Reset Password
                </button>
                <button type="submit" class="md-btn-submit"><i class="ti ti-device-floppy"></i> Update</button>
            </div>
        </div>
    </form>
</div>
