...
- **Fitur Chunked File Upload (Unggah Terfragmentasi Tanpa Timeout Server)**:
  - **Chunk Upload Controller ([`ChunkUploadController.php`](file:///c:/laragon/www/KELAS/lms_dani/app/Http/Controllers/Admin/ChunkUploadController.php))**:
    - Menerima potongan file via route `POST /admin/upload/chunk` dan menyimpannya di temporary directory `storage/app/chunks/{uuid}/`.
    - Menggabungkan seluruh bagian file secara berurutan saat chunk terakhir diterima, memvalidasi tipe file dan ekstensi aman (menolak arsip `.zip` dan `.rar`), lalu memindahkan file utuh ke target folder di disk public.
    - Dilengkapi endpoint pembersihan `POST /admin/upload/chunk/cancel` jika proses unggah dibatalkan.
  - **Komponen Vanilla JS Chunked Uploader Teroptimasi ([`public/js/chunked-uploader.js`](file:///c:/laragon/www/KELAS/lms_dani/public/js/chunked-uploader.js))**:
    - **Peningkatan Kecepatan Upload**: Ukuran chunk dioptimalkan menjadi 2.5MB per chunk sehingga memangkas roundtrip request secara signifikan (file 15MB selesai hanya dalam 6 request cepat).
